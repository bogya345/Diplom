...
namespace WebBRS.Controllers
{
	public class AccountController : Controller
    {
        ...
        public AccountController(IBrainstormSessionRepository sessionRepository)
        {
        ...
        [HttpPost("/token")]
        public JsonResult Token([FromForm] string username, [FromForm] string password)
        {
        ...
        private ClaimsIdentity GetIdentity(string username, string password)
        {
			...
			//if (!DAL.DAL_Settings.localAccess)
			{
				...
				person = unit.Users.Get(x => x.login == username && x.password == password);
			}
			//else
			{
				// local
            ...
            var claims = new List<Claim>
                {
					...
					new Claim(ClaimsIdentity.DefaultNameClaimType, person.Role.id_role.ToString())
